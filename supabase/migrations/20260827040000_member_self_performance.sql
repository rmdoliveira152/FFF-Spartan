...
declare
  ...
  current_week date := utc_today - extract(dow from utc_today)::integer;
  linked_member uuid;
  saved_snapshot uuid;
...
begin
  ...
  if requested_date not in (current_week, current_week - 7) then
    raise exception 'Only the current or previous weekly snapshot can be updated';
  end if;
